...
class MessengerGroupController extends APIController
{
  ...
  {
    $members =  MessengerMember::where('messenger_group_id', '=', $groupId)->orderBy('created_at', 'asc')->limit(3)->get(['account_id', 'status', 'messenger_group_id']);
    $j = 0;
    foreach ($members as $key) {
      $members[$j]['profile'] = app('Increment\Account\Http\AccountProfileController')->getAccountProfile($key['account_id']);
      $j++;
    }
    return $members;
  }
